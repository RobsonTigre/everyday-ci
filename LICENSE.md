# License

Copyright © 2025 by Robson Tigre. All rights reserved.

This repository contains the companion code and data for the book *Everyday Causal Inference* (https://www.everydaycausal.com/). It is **not** open-source software; it is proprietary material made publicly readable for the benefit of the book's readers.

## What you may do

- Read the book and run the code and data in this repository for personal learning.
- Use, modify, and adapt the code and data for your own analyses and professional projects, provided you give reasonable attribution where visible (e.g., "Based on examples from *Everyday Causal Inference* by Robson Tigre").
- Clone, fork, and download this repository through GitHub's standard features for the permitted uses above. This does not grant any right to republish or redistribute the material outside GitHub.
- Share official links to the book (https://www.everydaycausal.com/) or to this repository.
- Cite short excerpts with proper attribution.

## What you may not do

- Republish or redistribute the text, code, or data outside GitHub, in whole or in part.
- Use any part of this work to train AI or machine learning systems.
- Create competing courses, tutorials, or educational products based on this material.
- Build commercial training datasets from this content.
- Develop or market products using this book's name or its variants.

## Disclaimer

This content is for educational purposes only and does not constitute professional advice. All code is provided "as is," without warranty of any kind. The author disclaims all liability for outcomes based on this material.

See the full legal notice at https://www.everydaycausal.com/legal.html for details. If you have questions about permitted uses, contact the author via https://www.robsontigre.com/.
